...
export default function UpgradeModal({ trigger = "general", usage, onClose, onSuccess }: Props) {
  ...
  const redeemCoupon = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        setCouponError(data.error || "Invalid coupon. Please try again.");
      } else {
      ...
      setCouponError("Network error. Please check your connection.");
    }
            ...
            {activeTab === "upgrade" && (
              <>
                ...
                <p className="text-center text-xs text-muted-foreground">
                  Payment integration coming soon. Use a coupon code for free access.
                </p>
            ...
            {activeTab === "coupon" && (
              <div className="space-y-4">
                <div className="p-4 rounded-2xl bg-gradient-to-br from-violet-500/10 to-cyan-500/5 border border-violet-500/20 text-center">
                  ...
                  <p className="text-xs text-muted-foreground mt-1">Enter your coupon code below to get 7 days of free Premium access.</p>
                </div>
                ...
                <div className="space-y-2">
                  ...
                  {couponError && (
                    <motion.p initial={{ opacity: 0, y: -4 }} animate={{ opacity: 1, y: 0 }}
                      className="text-sm text-red-400 px-1">{couponError}</motion.p>
                  )}
