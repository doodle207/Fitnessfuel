...
import { User, Settings, LogOut, Activity, Target } from "lucide-react";
      ...
      <div className="max-w-2xl mx-auto space-y-8">
        <header className="text-center pb-8 border-b border-white/10">
          <div className="w-24 h-24 mx-auto bg-gradient-to-br from-primary to-accent rounded-full flex items-center justify-center mb-4 shadow-[0_0_30px_rgba(124,58,237,0.3)]">
            <span className="text-4xl font-display font-bold text-white">{profile.name.charAt(0)}</span>
          </div>
          <h1 className="text-3xl font-display font-bold">{profile.name}</h1>
          <p className="text-muted-foreground mt-1 capitalize">{profile.experienceLevel} • {profile.fitnessGoal}</p>
        </header>
