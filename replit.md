...
12. **UpgradeModal**: Full-screen modal with Upgrade Plan tab (Razorpay payment buttons) and Coupon tab (redeem code). Trigger types: `aiChat`, `scan`, `mealPlan`, `general`. Shows usage counters. Handles success/failure/cancel states.
13. **Pricing Page** (`/pricing`): Plan comparison (Free / Premium ₹199/mo / Pro ₹349/mo). Per-plan "Pay with Razorpay" buttons, coupon redemption, active plan status. Linked from desktop sidebar as "Premium" nav item.
15. **Razorpay Payment Gateway**: Full integration in INR. Backend: `POST /api/payments/razorpay/create-order` (creates order), `POST /api/payments/razorpay/verify` (HMAC SHA256 signature check → unlock premium in DB). `payment_history` table for audit trail. Frontend: loads Razorpay checkout.js dynamically, opens native checkout modal (UPI/Cards/Net Banking/Wallets). Secrets: `RAZORPAY_KEY_ID`, `RAZORPAY_KEY_SECRET` (both already set). Plans: Premium (19900 paise = ₹199) and Pro (34900 paise = ₹349), both 30 days.
