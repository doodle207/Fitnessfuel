...
12. **UpgradeModal**: Full-screen modal with Upgrade Plan tab (Razorpay/Stripe payment buttons) and Coupon tab (redeem code). India users → Razorpay (₹199/₹349). Non-India users → Stripe card checkout ($5.99/$9.99). Handles success/failure/cancel states.
13. **Pricing Page** (`/pricing`): Plan comparison (Free / Premium / Pro). Dynamically shows Razorpay for India users or Stripe for international. Handles Stripe redirect return (`?stripe_success=1&session_id=...`) — verifies session and activates plan. Cancel return shows amber notice. Security badge changes based on region.
15. **Razorpay Payment Gateway**: Full integration in INR. Backend: `POST /api/payments/razorpay/create-order`, `POST /api/payments/razorpay/verify` (HMAC SHA256). Secrets: `RAZORPAY_KEY_ID`, `RAZORPAY_KEY_SECRET` (both set). Receipt capped at 23 chars.
16. **Stripe Payment Gateway**: International card payments. Backend: `POST /api/payments/stripe/create-session` (redirect-based Stripe Checkout), `POST /api/payments/stripe/verify-session` (confirms paid status, activates plan). Secret needed: `STRIPE_SECRET_KEY`. Plans: Premium $5.99, Pro $9.99.
17. **Language Selector**: Dashboard header has a compact flag+globe icon button. Top 10 most spoken languages: English, Mandarin, Hindi, Spanish, French, Arabic, Bengali, Russian, Portuguese, Urdu. Greeting translates based on selected language. Preference saved to `cfx_language` in localStorage.
18. **Email OTP on Signup**: "Create Account" screen uses a two-step OTP flow: email → `POST /api/auth/email/request-otp` (sends 6-digit code via email) → OTP entry → `POST /api/auth/email/verify-otp` (creates user + session). 60-second resend timer. Back button to change email.
19. **Onboarding Draft Persistence**: All onboarding form data (step, age, height, weight, goals, etc.) is auto-saved to `cfx_onboarding_draft` in localStorage on every change. Restored on browser reopen. Draft cleared on successful profile submit.
20. **AI Food Scanner (Improved)**: `analyzeImageForFood` now uses `detail: "high"` + structured JSON return `[{name, grams}]` with visual portion estimation. `getMacrosForFood` scales OpenFoodFacts per-100g data to actual portion weight. `estimateFoodMacros` improved prompt with USDA/IFCT references, cooking method consideration, temperature 0.1.
